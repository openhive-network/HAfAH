...
Since HAF servers receive their data via a push model, they imposed a fixed amount of load on the hived node that supplies blockchain data, regardless of the number of HAF apps running on the server. In other words, while too many apps may load down the postgres database and affect the performance of other apps, the hived node supplying the data should continue to function without any problems.

![alt text](./doc/c2_haf.png)

The HAF contains a few components visible in the picture above:
* **HIVED - hive node**
  A HIVE node which syncs blocks with the HIVE peer-to-peer network or replays them from block_log file.
* **SQL_SERIALIZER**
...
* **PostgreSQL database**
  The database contains the data from blockchain blocks in the form of SQL tables and it also contains tables for the data generated by HAF applications running on the HAF server. The system utilizes Postgres authentication and authorization mechanisms to protect HAF-based applications from interfering with each other.
* **HIVE FORK MANAGER**
  **hive_fork_manager** is a PostgreSQL extension that implements the HAF's API, so this extension must be included when creating a new HAF database. The extension defines the format of block data saved in the database. It also defines a set of SQL stored procedures that are used by the application to get data about the blocks. The SQL_SERIALIZER dumps blocks to the tables defined by the hive_fork_manager. This extension defines the process by which HAF apps consume blocks, and ensures that apps cannot corrupt each other's data. The hive_fork_manager is also responsible for rewinding the state of the tables of all the HAF app running on the server in the case of a micro-fork occurrence. T
...
### Versioning
Postgres extensions are versioned - the extension control file contains `default_version` configuration entry. The build system fills the entry with the repository git sha.
Also, the corresponding SQL script file is named with the same version, as is required by postgres.
