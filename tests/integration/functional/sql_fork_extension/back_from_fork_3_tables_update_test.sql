...
BEGIN
    ...
    ASSERT ( SELECT COUNT(*) FROM hive.table2 WHERE id=223 AND smth='blabla2' AND hive_rowid=2 ) = 1, 'Updated row was not reverted table2';
    ASSERT ( SELECT COUNT(*) FROM hive.shadow_table2 ) = 0, 'Shadow table is not empty table2';

    ASSERT ( SELECT COUNT(*) FROM hive.table3 WHERE id=323 AND smth='blabla3' AND hive_rowid=3 ) = 1, 'Updated row was not reverted table3';
