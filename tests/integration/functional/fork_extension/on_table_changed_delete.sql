...
BEGIN
    ...
    DROP TRIGGER IF EXISTS on_src_table1_change_delete on table1;
    CREATE TRIGGER on_src_table1_change_delete AFTER DELETE ON table1
...
$BODY$
BEGIN
    DELETE FROM table1 WHERE smth = 2;
END
$BODY$
;
...
$BODY$
BEGIN
    ASSERT ( SELECT COUNT(*) FROM hive_tuples ) = 1, 'TEST FAILED';
    ASSERT ( SELECT COUNT(*) FROM hive_tuples WHERE operation = 2 ) = 1, 'TEST FAILED';
END
$BODY$
;
