...
BEGIN
    ...
    ASSERT EXISTS ( SELECT * FROM tuples ) = TRUE, 'ERROR: tuples are not saved by the trigger';
END
$BODY$
;
...
$BODY$
BEGIN
    ASSERT EXISTS ( SELECT FROM information_schema.tables WHERE table_name = 'src_table' ) = FALSE, 'ERROR: rollback transaction does not remove tables';
    -- check if rollback also rever changes in tuples table modified by the trigger
    ASSERT EXISTS ( SELECT * FROM tuples ) = FALSE, 'TEST FAILED';
END
$BODY$
;
