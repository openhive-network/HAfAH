...
BEGIN
    ...
    ASSERT ( SELECT current_block_num FROM hive.context WHERE name='context2' ) = 4, 'Wrong current block num';
    ASSERT ( SELECT events_id FROM hive.context WHERE name='context2' ) = 4, 'Wrong events id';
    ASSERT ( SELECT irreversible_block FROM hive.context WHERE name='context2' ) = 3, 'Wrong irreversible';
