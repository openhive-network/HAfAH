...
CASE WHEN _len > 0 AND _len = _limit THEN
  hafah_objects.do_pagination(_block_range_end, _limit, _len, ops_json, _fill_operation_id, _group_by_block, _id)
ELSE
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      hafah_objects.set_operation_id(next_operation_begin, _fill_operation_id, _id) AS next_operation_begin
    FROM (
      SELECT
