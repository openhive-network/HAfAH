...
ADD --chown=hafah_user:hafah_user ./scripts/setup_jmeter.bash .
ADD --chown=hafah_user:hafah_user ./scripts/setup_m2u.bash .
RUN chmod a+x ./setup_jmeter.bash ./setup_m2u.bash
RUN ./setup_jmeter.bash
RUN ./setup_m2u.bash
...
ENV PERFORMANCE_DIR=/home/hive/wdir
ENV LANG=en_US.UTF-8
...
# ENTRYPOINT [ "/home/hafah_user/performance_tests.bash" ]
