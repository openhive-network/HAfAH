...
if __name__ == '__main__':
  ...
  engine.add_argument('-u', '--use-direct-sql-as-condenser', dest='swap_direct', help='if given, condesner_api will be replaced with direct_sql and direct_sql will be disabled', **BOOL_PARAM)
  args = engine.parse_args(argv[1:])

  try:
    print("starting server, press ^C to stop server")
    run_server(args.psql, args.port, args.log_responses, Path(dirname(realpath(__file__))) / 'queries' / 'ah_schema_functions.pgsql', swap_condenser_direct=args.swap_direct)
