...
$function$
BEGIN
  RETURN ( SELECT hafah_python.get_bit_positions_64(_in, (SELECT id FROM hafd.operation_types WHERE is_virtual=TRUE ORDER BY id ASC LIMIT 1) ) );
END;
$function$
LANGUAGE plpgsql IMMUTABLE;
...
AND hafd.operation_id_to_type_id(hao.operation_id) = ANY(__resolved_filter)
