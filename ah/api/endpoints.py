...
def backend(args):
  return account_history_impl(args)
...
def get_ops_in_block(*, args, block_num : int = 0, only_virtual : bool = False, include_reversible : bool = DEFAULT_INCLUDE_IRREVERSIBLE, **kwargs):
  return build_response( backend(args).get_ops_in_block( block_num, only_virtual, include_reversible) )
...
def enum_virtual_ops(*, args, block_range_begin : int, block_range_end : int, operation_begin : int = 0, limit : int = MAX_INT_POSTGRES, filter : int = None, include_reversible : bool = DEFAULT_INCLUDE_IRREVERSIBLE, group_by_block : bool = False, **kwargs):
  assert block_range_end > block_range_begin, 'block range must be upward'
  return build_response( backend(args).enum_virtual_ops( filter, block_range_begin, block_range_end, operation_begin, limit, include_reversible, group_by_block ) )

@verify_types(convert_maybe)
def get_transaction(*, args, id : str, include_reversible : bool = DEFAULT_INCLUDE_IRREVERSIBLE, **kwargs):
  return build_response( backend(args).get_transaction( id, include_reversible ) )

@verify_types(convert_maybe, limit=(require_unsigned, limit_contraint))
def get_account_history(*, args, account : str, start : int = -1, limit : int = DEFAULT_LIMIT, operation_filter_low : int = 0, operation_filter_high : int = 0, include_reversible : bool = DEFAULT_INCLUDE_IRREVERSIBLE, **kwargs):
  filter = ( operation_filter_high << 64 ) | operation_filter_low
  start = start if start >= 0 else MAX_BIGINT_POSTGRES
  return build_response( backend(args).get_account_history( filter, account, start, limit, include_reversible ) )
