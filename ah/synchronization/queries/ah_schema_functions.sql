...
CREATE OR REPLACE FUNCTION hafah_python.ah_get_account_history( in _filter INT[], in _account VARCHAR, _start BIGINT, _limit INT, in _include_reversible BOOLEAN, in _is_legacy_style BOOLEAN )
RETURNS TABLE(
  _trx_id TEXT,
  _block INT,
  _trx_in_block BIGINT,
