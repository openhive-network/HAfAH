...
docker build --target=base_instance \
  ...
  --build-arg BUILD_IMAGE_TAG=$BUILD_IMAGE_TAG -t ${REGISTRY}base_instance$BUILD_IMAGE_TAG -f Dockerfile .
...
docker build --target=instance \
  --build-arg BLOCK_LOG_SUFFIX=${BLOCK_LOG_SUFFIX:-""} \
  --build-arg CI_REGISTRY_IMAGE=$REGISTRY --build-arg CI_IMAGE_TAG=$CI_IMAGE_TAG \
  --build-arg BUILD_IMAGE_TAG=$BUILD_IMAGE_TAG -t ${REGISTRY}instance$BUILD_IMAGE_TAG -f Dockerfile .
